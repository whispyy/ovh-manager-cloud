(() => {
    ...
    class VeeamService {
        ...
        transformConfigurationInfos (infos) {
            infos.detail.location = this.RegionService.getRegion(infos.detail.location);
            infos.detail.backupCount = infos.inventoryNames.length;
            return infos.detail;
        }
        ...
        getStorages (serviceName) {
            return this.veeam.getInventories({
                ...
                const storagesPromises = storages.map(storage => this.getStorageDetails(serviceName, storage));
        ...
        transformSubscriptionInfos (data) {
            ...
            data.serviceInfo.offer = data.details.productOffer;
            data.serviceInfo.renewalType = renewalType;
            data.serviceInfo.renewalTypeDescription = this.$translate.instant(`veeam_infos_subscription_renew_${renewalType}_description`);

            data.serviceInfo.isOnTrial = data.serviceInfo.offer === "demo";
            // eslint-disable-next-line
            data.serviceInfo.subscriptionTimeRemaining = (new moment(data.serviceInfo.expiration)).diff(new moment(), "days");
        ...
        getOrderableOfferPrices (serviceName) {
            return this.veeam.getOrderableOffers({
                ...
                .then(durations => {
                    const promises = _.map(durations, duration => this.getUpgradeOptionPrices(serviceName, duration.offer, duration.duration)
                        .then(price => ({ offer: duration.offer, duration: duration.duration, price: price.data })));
        ...
        checkTasks (serviceName, tasks) {
            ...
            if (completedTasksIds.length) {
                const completedTasks = this.unitOfWork.tasks.filter(task => completedTasksIds.indexOf(task.taskId) >= 0);
